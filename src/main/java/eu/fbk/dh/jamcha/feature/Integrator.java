...
public class Integrator
{
  ...
  /**
   * All the features of the training file with additions resulting from the integration process, performed with tuning
   * parameters passed to the integrated method
   */
  @Nonnull
   ...
   * parameters)
   *
   * @param requestedline line from which we want to extract features. Must be between 0(inclusive) and lines count
   *                      (exclusive). ATTENTION: there is no value check
...
//      ? line.getWords().get(index) : line.getTag();
      if (index != FeatureParser.TAG_COLUMN_INDEX && index< line.getWords().size())
      {
        builder.append("f").append(index).append("_").append(offset).append("_").append(line.getWords().get(index));
      }
      else
      {
        builder.append("r_").append(offset).append("_").append(line.getTag());
